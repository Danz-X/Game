    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'Danzbotz' : banned ? 'TELAH DI BANNED' : 'DanzBotz',
                '©Raditya',
...
`.trim(), '© Danzbotz', 'CHAT ME TO BUY', ',owner', { contextInfo: { mentionedJid: [global.owner[0] + '@s.whatsapp.net'] } })
    }
...
// tes
    ...
    if (isF && !m.fromMe) {       
        let prom = 'YouTube : https://bit.ly/\n\nInstagram : https://bit.ly/ly.'
